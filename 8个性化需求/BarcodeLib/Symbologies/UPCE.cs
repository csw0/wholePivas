...
namespace BarcodeLib
{
    class UPCE : BarcodeCommon, IBarcode
    {
        ...
        {
            if (Raw_Data.Length != 6 && Raw_Data.Length != 8 && Raw_Data.Length != 12) throw new Exception("EUPCE-1: Invalid data length. (8 or 12 numbers only)");

            if (!BarcodeLib.Barcode.CheckNumericOnly(Raw_Data)) throw new Exception("EUPCE-2: Numeric only.");
