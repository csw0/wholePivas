...
namespace BLPublic
{
    public class OLEDB_Oracle_SQLServer
    {
        private OleDbConnection conn;               // 数据库连接
        private OleDbCommand cmd;                   // 数据库执行的SQL命令
        private OleDbTransaction transaction;       // 数据库事务

        public string DBType { get; private set; }  // 连接的OLEDB数据库类型

        //
        // 初始化conn，并打开数据库连接
        public void Connect(string configFilePath)
        {
            string connectionString = "";
            try
            {
                // 从CPMATE连接文件（扩展名lcf，格式为XML）中读取各项信息
                // 如果发生文件未找到异常，说明lcf连接文件丢失
                // 如果发生空指针异常，说明lcf连接文件内容不全
                XDocument config = XDocument.Load(configFilePath);
                XElement xeCPMATE = config.Element("lxf").Element("CPMATE");

                // 首先从配置文件中获得数据库类型，转换为小写字母
                this.DBType = xeCPMATE.Element("db_type").Value.ToLower();

                // 获取其他连接数据库的信息
                string server_name = xeCPMATE.Element("server_name").Value;
                string user_name = xeCPMATE.Element("user_name").Value;
                string encryptedPassword = xeCPMATE.Element("password").Value;
                string password = this.Decrypt(encryptedPassword, "PASSWORDKEY");
                string database = xeCPMATE.Element("database").Value;
                ...
                switch (this.DBType)
                {
                    case "oracle":
                        {
                            ...
                            connectionString = string.Format(
                                "Provider=OraOLEDB.Oracle;Data Source={0};User ID={1};Password={2};"
                    ...
                    case "sqlserver":
                        {
                            // 使用SQL Server的OLEDB驱动
                            // 不使用保存密码：Persist Security Info=True
                            connectionString = string.Format(
                                "Provider=SQLOLEDB;Data Source={0};User ID={1};Password={2};"
                              + "Initial Catalog={3};"
                              , server_name, user_name, password, database);
                            break;
                        }

                    default:
                        {
                            string errorMessage = "CPMATE的lcf连接设置文件内容不正确：\n"
                                                + "<db_type>没有指定数据库为oracle或sqlserver！"
                                                + "当前值：<db_type>" + this.DBType + "</db_type>\n"
                                                + configFilePath;
            ...
            {
                string errorMessage = "找不到CPMATE的lcf连接设置文件，文件路径："
                                    + configFilePath;
                throw new Exception(errorMessage, ex);
            }
            catch (System.IO.IOException ex)
            {
                string errorMessage = "读取CPMATE的lcf连接设置文件错误，文件路径："
                                    + configFilePath;
                throw new Exception(errorMessage, ex);
            }
            catch (NullReferenceException ex)
            {
                string errorMessage = "CPMATE的lcf连接设置文件内容不完整，请检查文件内容："
                                    + configFilePath;
                throw new Exception(errorMessage, ex);
            }
        ...
        /// <summary>
        /// 根据数据库连接字符串和数据库类型，连接数据库
        /// </summary>
        /// <param name="connectionString">数据库连接字符串</param>
        /// <param name="dbType">dbType分为：oracle和sqlserver</param>
        public void Connect(string connectionString, string dbType)
        {
        ...
        private void BeginTransaction()
        {
            ...
            {
                string errorMessage = "尝试创建数据库事务（OleDbTransaction）失败";
                throw new Exception(errorMessage, ex);
            }
        }
        ...
        private void BeginCommand()
        {
            ...
            {
                string errorMessage = "尝试创建数据库命令（OleDbCommand）失败";
                throw new Exception(errorMessage, ex);
            }
        }
        ...
        // 每个操作数据库的方法完毕后：
        // 关闭数据库命令cmd
        // 命令cmd和事务transaction置为null
        //
        // transaction的关闭放在Commit()或Rollback()中
        ...
        public OleDbDataReader GetRecord(string sql)
        {
            try
            {
                this.BeginCommand();
        ...
        public long ExecuteInsertReturnID(string insert)
        {
            try
            {
                ...
                using (OleDbDataReader reader = this.cmd.ExecuteReader())
                {
                    if (!reader.Read())
                    {
        ...
        //
        // 更新CLOB类型的数据
        // CLOB类型无法直接插入，先插入null，再通过ID等唯一标识更新
        // CLOB类型无法使用SQL语句直接更新，使用参数化更新
        public int UpdateClob(string update, List<string> clobValues)
        {
            try
            {
                this.BeginTransaction();

                // 设置UPDATE语句，和CLOB的参数值
                this.BeginCommand();
                this.cmd.CommandText = update;

                // CLOB对应OLEDB中的LongVarChar类型
                // 按顺序插入参数（Insert方法顺序插入参数），不额外指定参数名（不使用Add方法）
                for (int i = 0; i < clobValues.Count; i++)
                {
                    OleDbParameter parameter = new OleDbParameter();
                    parameter.OleDbType = OleDbType.LongVarChar;
                    parameter.Value = clobValues[i];
                    this.cmd.Parameters.Insert(i, parameter);
                }
                this.cmd.CommandText = update;
                int rowCount = this.cmd.ExecuteNonQuery();
        ...
        //
        // 仅更新一个CLOB字段的简化方法
        public int UpdateClob(string update, string clobValue)
        {
            List<string> clobValues = new List<string>();
            clobValues.Add(clobValue);
            return this.UpdateClob(update, clobValues);
        }
