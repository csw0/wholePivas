    ...
    public partial class CancelMessage : UserControl
    {
        ...
        public void setmsg(string Type,DataRow row) 
        {
            ...
            else if(Type=="提前打包")
            {
                lb_name.Text = "提前打包 :";
                lb_name.ForeColor = Color.Red;
                if (row["PackName"].ToString().Trim() == "" && row["PackID"].ToString().Trim() == "")
                {
                    User.Text = "未知";
                }
                else if (row["PackName"].ToString().Trim() == "" && row["PackID"].ToString().Trim() != "")
                {
                    User.Text = row["PackID"].ToString();
                }
                else if (row["PackName"].ToString().Trim() != "")
                {
                    User.Text = row["PackName"].ToString() + "(" + row["PackDEmpCode"].ToString() + ")";
                }
                lb_time.Text = row["PackTime"].ToString();
                lb_Type.Text = "";
                lb_Location.Text ="";
                lb_Location.Visible =false;
            }
            ...
            {
                if (row["核对"].ToString() == "审方" || row["核对"].ToString() == "打印")
                {
