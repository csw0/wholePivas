...
{
    public partial class PackCollectForm : Form
    {
        public PackCollectForm()
        {
            InitializeComponent();
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                report.Load(".\\Crystal\\PackCollect.frx");
        ...
        {
            string sql = "  select count (distinct labelno) ,wardcode ,dbprintwardcode from [IVRecord_DBPrint] group by WardCode,dbprintwardcode";
            return db.GetPIVAsDB(sql).Tables[0].Rows[0]["dbprintwardcode"].ToString();
        ...
        private DataTable GetBed(string Dward)
        {
            ...
            str.Append(" and PrintDT between '" + b + "' and '" + a + "' ");
        ...
        private DataSet GetPackCollect()
        {
            ...
            else
            {
                ...
                if (radioButton2.Checked)//已打印
                {
                    ...
                    str.Append("    inner join IVRecord_DBPrint dbp on IVRecord.LabelNo =dbp.LabelNo");
                ...
                else if (radioButton1.Checked)//未打印
                {
                    ...
                    str.Append(" (select  WardName,WardCode,'第'+convert(varchar,TeamNumber)+'批' as Batch,COUNT(distinct LabelNo) as AllCount from IVRecord where IVRecord.LabelNo in (select IVRecordID from IVRecord_DB except select labelno from IVRecord_DBPrint)  " + pubstr2 + " group by WardCode,WardName,TeamNumber )a ");
                    str.Append(" left join  ");
                    str.Append(" (select WardName,IVRecord.WardCode,'第'+convert(varchar,TeamNumber)+'批' as Batch,count(distinct LabelNo) as ConfectCount from IVRecord where IVRecord.LabelNo in (select IVRecordID from IVRecord_DB except select labelno from IVRecord_DBPrint) " + pubstr2 + " and  Batch like '%#%' and Batch not like '%l%' group by IVRecord.WardCode,WardName,TeamNumber)b ");
                    str.Append(" on a.WardCode=b.WardCode and a.Batch=b.Batch ");
                    str.Append(" left join ");
                    str.Append(" (select WardName,WardCode,'第'+convert(varchar,TeamNumber)+'批' as Batch,count(distinct LabelNo)as EmptyCount from IVRecord where IVRecord.LabelNo in (select IVRecordID from IVRecord_DB except select labelno from IVRecord_DBPrint) " + pubstr2 + " and Batch like '%K%' and Batch not like '%l%'   group by WardCode,WardName,TeamNumber)c");
                    str.Append(" on a.WardCode=c.WardCode and a.Batch=c.Batch ");
                    str.Append(" left join ");
                    str.Append(" (select WardName,WardCode,'第'+convert(varchar,TeamNumber)+'批' as Batch,count(distinct LabelNo)as IsCommandCount from IVRecord where IVRecord.LabelNo in (select IVRecordID from IVRecord_DB except select labelno from IVRecord_DBPrint) " + pubstr2 + " and Batch like '%L%' group by WardCode,WardName,TeamNumber)d ");
                ...
                else //全部
                {
                    ...
                    str.Append("    ,count (case when Batch like '%L%' then 1  end ) as IsCommandCount");
                    str.Append("   from IVRecord ");
                    str.Append("    inner join IVRecord_DB db on IVRecord.LabelNo =db.IVRecordID");
                    str.Append("   where 1=1" + pubstr2 + " and ScanCount='0'");
                    str.Append("   group by WardName,TeamNumber");
                }
        ...
        private DataTable GetDrugtype(string WardCode)
        {
            ...
            StringBuilder str = new StringBuilder();
            DataSet ds = new DataSet();
            string pubstr1 = SelectRule1(startTime, endTime, WardCode);//获取界面上的时间和病区
            ...
            if (radioButton1.Checked)
            {
                str.Append(" and IVRecord.LabelNo in (select IVRecordID from IVRecord_DB except select labelno from IVRecord_DBPrint) ");
            }
        ...
        private void addDBCode()
        {
            ...
            if (radioButton1.Checked)//未打印
            {
                for (int i = 0; i < ward.Rows.Count; i++)
                {
                    ...
                    string sql = "select distinct db.IVRecordID from IVRecord_DB db left join IVRecord   on db.IVRecordID=IVRecord.LabelNo where 1=1 " + SelectRule1(startTime, endTime, getWardCodeByWardName(wardName)) + " ";
                   ...
                   // sql = string.Format(sql, datepick.Value, wardName);
                    ...
                    for (int row = 0; row < ds.Tables[0].Rows.Count; row++)
                    {
                        ...
                        string sqlIn = "insert into IVRecord_DBPrint values ('" + lableNo + "'," + getWardCodeByWardName(wardName) + ",'" + DateTime.Now.ToString() + "','','0','" + printcode + "','" + DBCode + "','')";
                        db.SetPIVAsDB(sqlIn);
                    }
                }
            }
            else if (radioButton2.Checked)//已打印
            {
                for (int i = 0; i < ward.Rows.Count; i++)
                {
                    DataRow r = ward.Rows[i];
                    string wardName = r["WardName"].ToString();
                    string sql = "select distinct dbp.LabelNo from IVRecord_DBPrint dbp left join IVRecord   on dbp.LabelNo=IVRecord.LabelNo where 1=1 " + SelectRule1(startTime, endTime, getWardCodeByWardName(wardName)) + " ";
                    //sql += "";
                    ...
                    for (int row = 0; row < ds.Tables[0].Rows.Count; row++)
                    {
                        ...
                        string str = "select Max(ScanCount) from [IVRecord_DBPrint] where LabelNo='" + lableNo + "'";
                        int n = Convert.ToInt32(db.GetPIVAsDB(str).Tables[0].Rows[0][0].ToString()) + 1;
                        string sqlIn = "insert into IVRecord_DBPrint values ('" + lableNo + "'," + getWardCodeByWardName(wardName) + ",'" + DateTime.Now.ToString() + "','','" + n.ToString() + "','" + printcode + "','" + DBCode + "','')";
                        db.SetPIVAsDB(sqlIn);
                    }
                }
            }
        ...
        private string SelectRule1(string Date, string date1, string wardcode)
        {
            ...
            if ((dateTimePicker1.Value- datepick.Value).Days==0)
            {
            ...
            foreach(Control cb in panel3.Controls)
            {
                if (cb is CheckBox)
                {
                    CheckBox _cb = cb as CheckBox;
                    if(_cb.Checked)
                    strrule.Append(string.Format(" or  Batch like '%{0}%'", _cb.Text));
                }
            }
            strrule.Append(") ");
        ...
        private void Panel_Max_Click(object sender, EventArgs e)
        {
            if (this.WindowState == System.Windows.Forms.FormWindowState.Normal)
            {
                ...
                Panel_Max.BackgroundImage = global::PackCollect.Properties.Resources.还原;

            }
            else
            {
                this.WindowState = FormWindowState.Normal;
                Panel_Max.BackgroundImage = global::PackCollect.Properties.Resources._20;
            }
        }
