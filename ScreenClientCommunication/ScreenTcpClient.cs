...
namespace Communication.DisplayTcp
{
    ...
    /// 负责数据的接收和发送
    /// </summary>
    public class ScreenTcpClient : IDisposable
    {
        #region 私有
        ...
        public ScreenTcpClient(string remoteIP, int remotePort, int bufferSize = 8192)
        {
        ...
        protected void Client_Connected(object sender, AsyncSocketSessionEventArgs e)
        {
            try
            {
                InternalLogger.Log.Info(String.Format("ScreenTcpClient({0}:{1}[{2}])已经连接成功:",
                    this.serverIp, this.serverPort, this.sessionId));
                connectStatus = true;
                if (Connected != null)
                {
        ...
        protected void Client_Disconnected(object sender, AsyncSocketSessionEventArgs e)
        {
            try
            {
                InternalLogger.Log.Info(String.Format("ScreenTcpClient({0}:{1}[{2}])已经断开连接:",
                    this.serverIp, this.serverPort, this.sessionId));
                connectStatus = false;
        ...
        void ParseMoxaData(string data)
        {
            string[] labelNos = Regex.Split(data, StaticDictionary.SPLITTER, RegexOptions.IgnoreCase);
            foreach (var item in labelNos)
            {
                //将瓶签号数据交由其由事件处理
                if (DataReceived != null)
                {
                    DataReceived(this, new PivasEventArgs<TCPMessage>(new TCPMessage(this.serverIp, this.serverPort, item)));
                }
                InternalLogger.Log.Info("接收到已分包完成的瓶签号数据：" + item);
            }
        }
        ...
        {
            InternalLogger.Log.Error(String.Format("ScreenTcpClient({0}:{1}[{2}])通信过程中出错:",
        ...
        protected void Client_Started(object sender, EventArgs e)
        {
            try
            {
                InternalLogger.Log.Info(String.Format("ScreenTcpClient({0}:{1}[{2}])尝试去打开连接:",
                this.serverIp, this.serverPort, this.sessionId));
            }
            catch (Exception ex)
            {
        ...
        protected void Client_Stopped(object sender, EventArgs e)
        {
            try
            {
                InternalLogger.Log.Info(String.Format("ScreenTcpClient({0}:{1}[{2}])尝试去断开连接:",
                this.serverIp, this.serverPort, this.sessionId));
            }
            catch (Exception ex)
            {
        ...
        public bool Send(string displayData )
        {
            try
            {
                return client.Send(Encoding.Default.GetBytes(displayData));
            }
            catch (Exception ex)
            {
